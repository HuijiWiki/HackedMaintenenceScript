...
class InsertSiteRank extends Maintenance {
	public function execute() {
		...
		foreach ($allSite as $value) {
			...
			$editUserDate[$value] = round(isset($editUserYesterday[$value])?$editUserYesterday[$value]:0+(isset($editUserWeek[$value])?$editUserWeek[$value]:0)/3+(isset($editUserMonth[$value])?$editUserMonth[$value]:0)/10);
		}
		//sort arr
		asort($viewDate);
		asort($editDate);
		...
		foreach ($allRank as $key => $value) {
			...
			$site_rank = (!is_null($key_rank))?$key_rank:999999;
			if( $rank <  $site_rank ){
				$dbw = wfGetDB( DB_MASTER );
				$dbw->upsert(
					'site_best_rank',
